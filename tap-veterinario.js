(() => {
  ...
  generate.addEventListener('click', event => {
    const type = activity.value;
    const realLogo = Boolean(logoDataUrl) && !window.tapLogoSkipped;
  ...
  preview.addEventListener('click', async event => {
    ...
    try {
      ...
      const hasLogo = Boolean(logoDataUrl) && !window.tapLogoSkipped;
      const placeholderPixel = NO_LOGO_PIXEL;
      let html = '';
