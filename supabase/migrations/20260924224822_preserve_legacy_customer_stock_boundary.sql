alter table public.inventario_movimenti drop constraint if exists inventario_movimenti_non_vuoti;
alter table public.inventario_movimenti add constraint inventario_movimenti_non_vuoti
  check (tipo in ('apertura','vendita') or targhe <> 0 or carte <> 0 or adesivi <> 0);
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    select exists (select 1 from public.inventario_movimenti
      where source_client_id = old.id and tipo in ('vendita','rettifica_vendita')) into cliente_tracciato;
    if not cliente_tracciato then return new; end if;
